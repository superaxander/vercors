...
case class LangCPPToCol[Pre <: Generation](rw: LangSpecificToCol[Pre]) extends LazyLogging {
  ...
  private def rewriteSYCLQueueSubmit(invocation: CPPInvocation[Pre]): (Block[Post], Variable[Post]) = {
    ...
    val preEventClass: Class[Pre] = new ByValueClass(Nil, Nil, Nil)(commandGroup.o)
    ...
    val postEventClass = new ByValueClass[Post](
      typeArgs = Seq(),
    ...
    val eventClassRef = new Variable[Post](TByValueClass(postEventClass.ref, Seq()))(commandGroup.o.where(name = "sycl_event_ref"))
