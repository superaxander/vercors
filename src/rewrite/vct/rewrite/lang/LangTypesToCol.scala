    ...
    t match {
      ...
      case t @ PVLNamedType(_, typeArgs) =>
        t.ref.get match {
          ...
          case RefClass(decl: ByReferenceClass[Pre]) => TByReferenceClass(succ[Class[Post]](decl), typeArgs.map(dispatch))
          case RefClass(decl: ByValueClass[Pre]) => TByValueClass(succ[Class[Post]](decl), typeArgs.map(dispatch))
        }
