      ...
      stat match {
          ...
          if(breakLabelException.contains(decl)) {
            TryCatchFinally(
              ...
              catches = Seq(CatchClause(
                decl = new Variable(TByReferenceClass(breakLabelException.ref(decl), Seq())),
                body = Block(Nil),
              )),
            )
          } else {
        ...
        case Break(Some(Ref(label))) =>
          val cls = breakLabelException.getOrElseUpdate(label,
            globalDeclarations.declare(new ByReferenceClass[Post](Nil, Nil, Nil, tt)(BreakException)))
          ...
          allScopes.anyDeclare(allScopes.anySucceedOnly(method, method.rewrite(body = Some({
            if (needBreakReturnExceptions(body)) {
              ...
              if(needReturn(method)) {
                ...
                val returnClass = new ByReferenceClass[Post](Nil, Seq(returnField), Nil, tt)(ReturnClass)
                globalDeclarations.declare(returnClass)

                val caughtReturn = new Variable[Post](returnClass.classType(Seq()))
