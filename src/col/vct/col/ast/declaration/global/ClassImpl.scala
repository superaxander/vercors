...
import vct.col.ast.{Class, ClassDeclaration, Declaration, Expr, TByReferenceClass, TClass, TVar, Type, Variable}
...
trait ClassImpl[G] extends Declarator[G] { this: Class[G] =>
  ...
  def transSupportArrowsHelper(seen: Set[TClass[G]]): Seq[(TClass[G], TClass[G])] = {
    ...
    val t: TClass[G] = classType(typeArgs.map((v: Variable[G]) => TVar(v.ref[Variable[G]])))
